...
def get_headers():
    token = (st.secrets.get("HUGGINGFACEHUB_API_TOKEN")
             ...
             or "").strip()
    ok = token.startswith("hf_")
    return ({"Authorization": f"Bearer {token}"} if ok else {}), ok

HF_HEADERS, TOKEN_OK = get_headers()
...
# =================== IA helpers (robustos) ===================
def _post_with_retry(url, json, headers, timeout=60, retries=3):
    last = None
    for k in range(retries+1):
        try:
            ...
            last = str(e)
            time.sleep(1.5*(k+1))
    raise RuntimeError(last or "sin detalle")
...
def hf_zero_shot(text, labels=LABELS, headers=None):
    ...
    out = _post_with_retry(url, payload, headers, timeout=60, retries=3)
...
def extract_fields_from_relato(text: str):
...
def render():
    ...
    DEBUG     = st.sidebar.toggle("🔧 Diagnóstico", value=False)
    N_MAX     = st.sidebar.number_input("Máximo de filas a procesar", 1, 200, 6)
    st.sidebar.write("Token OK:", TOKEN_OK)

    if DEBUG:
        if SAFE_MODE: st.warning("Modo seguro activo: no se llamará a la IA.")
        if not TOKEN_OK: st.error("No hay token HF válido (hf_...).")
        st.caption(f"Headers activos: {'sí' if HF_HEADERS else 'no'}")
    ...
    with tabs[1]:
        ...
        file = st.file_uploader("Sube CSV con columnas: rama,grado,unidad,provincia,canton,fecha_incidente,relato",
                                type=["csv"])
        if not file:
        ...
        try:
            ...
            if not req.issubset(df.columns):
                st.error(f"Faltan columnas: {req - set(df.columns)}"); return
            ...
            for i, row in df.iterrows():
                texto = str(row["relato"])
                if SAFE_MODE or not HF_HEADERS:
                    zs, tipo, score = [], "otros", 0.0
                else:
                    try:
                        zs = hf_zero_shot(texto, headers=HF_HEADERS)
                        tipo, score = (zs[0][0], float(zs[0][1])) if zs else ("otros", 0.0)
                    except Exception as e:
                        if DEBUG: st.warning(f"[Zero-shot fila {i+1}] {e}")
            ...
            st.dataframe(df[["relato","tipo_predicho","score_tipo"]], use_container_width=True)
            ...
            for i, row in df.iterrows():
                ...
                if SAFE_MODE or not HF_HEADERS:
                    ...
                    try:
                        ents = hf_ner(texto_ner, headers=HF_HEADERS)
                    except Exception as e:
                        if DEBUG: st.warning(f"[NER fila {i+1}] {e}")
                        ents = []

                ALLOWED = {"ORG","PER","LOC","MISC"}
                ents_fmt = [f"{w}({t})" for (w,t,_s) in ents if t in ALLOWED]
                # evitar duplicados conservando orden
                ents_fmt = list(dict.fromkeys(ents_fmt))
            ...
            st.dataframe(df[["relato","entidades","montos","fechas"]], use_container_width=True)

            # ===== 3) Riesgo =====
            st.markdown("### 3) Prioridad (Reglas de riesgo)")
            riesgos = []
            for _, row in df.iterrows():
                raw_dates = extract_fields_from_relato(str(row["relato"]))["fechas"]
                riesgos.append(risk_score(row["tipo_predicho"], str(row["relato"]), raw_dates, row["fecha_incidente"]))
            df["riesgo"] = riesgos
